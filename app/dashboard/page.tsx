...
export default function Home() {
  ...
  useEffect(() => {
    if (selectedDistributor !== '') {
      const selectedSectors = getSectors().then((backendSectors: any) => {
        const filteredSectorData: Sector[] = backendSectors.filter((s: any) => s.dist_code === selectedDistributor);
  ...
  const handleDistributorChange = (event: ChangeEvent<HTMLSelectElement>) => {
    ...
    const selectedSectors = getSectors().then((backendSectors: any) => {
      const filteredSectorData: Sector[] = backendSectors.filter((s: any) => s.dist_code === selectedDistributorCode);
      setFilteredSectors(filteredSectorData);
    });
  };
  ...
  const handleSectorChange = (event: ChangeEvent<HTMLSelectElement>) => {
    ...
    const selectedSectors = getSectors().then((backendSectors: any) => {
      const filteredSectorData: Sector[] = backendSectors.filter((s: any) => s.dist_code === selectedDistributor);
      const selectedSector = filteredSectorData.find((sector) => sector.seccd === selectedSectorCode);
      if (selectedSector) {
